...
function ServicesPage({ onBook, reviews, onAddReview }) {
  ...
  const trackBooking = async () => {
    ...
    setSearching(true);
    try {
      const data = await apiFetch("/bookings/track/" + phone);
      setMyBookings(data);
    } catch(e) {
      setMyBookings([]);
  ...
  return (
    <div style={{ maxWidth:960, margin:"0 auto", padding:"32px 16px 80px" }}>
      ...
      {/* MY BOOKINGS TRACKER */}
      <div style={{ background:C.card, border:"1px solid " + C.border, borderRadius:16, padding:22, marginBottom:44 }}>
        <p style={{ fontFamily:SF, fontSize:11, color:C.gold, letterSpacing:6, marginBottom:6 }}>BOOKING STATUS</p>
        <h3 style={{ fontFamily:SF, fontSize:20, color:C.cream, fontWeight:600, marginBottom:14 }}>Track My Booking</h3>
        <div style={{ display:"flex", gap:8, marginBottom:14 }}>
          <input value={phone} onChange={e => setPhone(e.target.value)} placeholder="Enter your phone number" style={{ ...SI, flex:1 }} />
          <Btn onClick={trackBooking} style={{ padding:"10px 16px" }}>{searching ? "..." : "Track"}</Btn>
        </div>
        {myBookings.length === 0 && phone && !searching && (
          <p style={{ color:C.faint, fontSize:13, textAlign:"center" }}>No bookings found for this number</p>
        )}
        {myBookings.map(b => (
          <div key={b._id} style={{ background:C.surface, border:"1px solid " + (b.status === "confirmed" ? C.green : b.status === "cancelled" ? C.red : C.gold), borderRadius:12, padding:16, marginBottom:10 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
              <div>
                <p style={{ color:C.cream, fontWeight:700 }}>{b.date} at {b.time}</p>
                <p style={{ color:C.muted, fontSize:12 }}>👥 {b.guests} guests</p>
              </div>
              <div style={{ textAlign:"center" }}>
                <div style={{ fontSize:28 }}>{b.status === "confirmed" ? "✅" : b.status === "cancelled" ? "❌" : "⏳"}</div>
                <div style={{ color: b.status === "confirmed" ? "#81C784" : b.status === "cancelled" ? "#E57373" : C.gold, fontSize:11, fontWeight:700 }}>
                  {b.status === "confirmed" ? "CONFIRMED" : b.status === "cancelled" ? "REJECTED" : "PENDING"}
                </div>
              </div>
            </div>
            {b.status === "confirmed" && (
              <div style={{ background:"#082008", border:"1px solid " + C.green, borderRadius:8, padding:"8px 12px" }}>
                <p style={{ color:"#81C784", fontSize:12, fontWeight:600 }}>🎉 Your table is confirmed! We look forward to seeing you.</p>
              </div>
            )}
            {b.status === "cancelled" && (
              <div style={{ background:"#200808", border:"1px solid #5A1010", borderRadius:8, padding:"8px 12px" }}>
                <p style={{ color:"#E57373", fontSize:12, fontWeight:600 }}>We're sorry, your booking was not accepted. Please try another date/time.</p>
              </div>
            )}
            {b.status === "pending" && (
              <div style={{ background:"#1A1000", border:"1px solid " + C.goldD, borderRadius:8, padding:"8px 12px" }}>
                <p style={{ color:C.gold, fontSize:12, fontWeight:600 }}>⏳ Your booking is under review. We'll confirm it soon!</p>
              </div>
            )}
          </div>
        ))}
    ...
    <div style={{ maxWidth:1100, margin:"0 auto", padding:"32px 16px 80px" }}>
      ...
      {tab === "bookings" && (
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          ...
          {bookings.map(b => (
            <div key={b._id} style={{ background:C.card, border:"1px solid " + (b.status === "pending" ? C.gold : b.status === "confirmed" ? C.green : C.red), borderRadius:12, padding:"16px 18px" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:8 }}>
                <div>
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:4 }}>
                    <span style={{ color:C.cream, fontWeight:700, fontSize:14 }}>{b.name}</span>
                    <span style={{ background: b.status === "pending" ? C.gold + "22" : b.status === "confirmed" ? "#0A200A" : "#200808", color: b.status === "pending" ? C.gold : b.status === "confirmed" ? "#81C784" : "#E57373", fontSize:10, padding:"2px 10px", borderRadius:20, fontWeight:700 }}>
                      {b.status === "pending" ? "⏳ PENDING" : b.status === "confirmed" ? "✅ CONFIRMED" : "❌ REJECTED"}
                    </span>
                  </div>
                  <p style={{ color:C.muted, fontSize:12 }}>📞 {b.phone} · 👥 {b.guests} guests</p>
                  <p style={{ color:C.gold, fontSize:12, marginTop:2 }}>📅 {b.date} at {b.time}</p>
              ...
              {b.status === "pending" && (
                <div style={{ display:"flex", gap:8, marginTop:10 }}>
                  <Btn onClick={async () => {
                    const updated = await apiFetch("/bookings/" + b._id, { method:"PUT", body: JSON.stringify({ status:"confirmed" }) });
                    setBookings(prev => prev.map(x => x._id === b._id ? updated : x));
                  }} variant="success" style={{ flex:1, padding:"9px 0" }}>✅ Accept Booking</Btn>
                  <Btn onClick={async () => {
                    const updated = await apiFetch("/bookings/" + b._id, { method:"PUT", body: JSON.stringify({ status:"cancelled" }) });
                    setBookings(prev => prev.map(x => x._id === b._id ? updated : x));
                  }} variant="danger" style={{ flex:1, padding:"9px 0" }}>❌ Reject Booking</Btn>
                </div>
              )}
              {b.status !== "pending" && (
                <div style={{ display:"flex", gap:8, marginTop:10 }}>
                  <Btn onClick={async () => {
                    const updated = await apiFetch("/bookings/" + b._id, { method:"PUT", body: JSON.stringify({ status:"pending" }) });
                    setBookings(prev => prev.map(x => x._id === b._id ? updated : x));
                  }} variant="ghost" style={{ padding:"7px 14px", fontSize:11 }}>↩️ Reset</Btn>
                  <Btn onClick={async () => {
                    await apiFetch("/bookings/" + b._id, { method:"DELETE" });
                    setBookings(prev => prev.filter(x => x._id !== b._id));
                  }} variant="danger" style={{ padding:"7px 14px", fontSize:11 }}>🗑️ Delete</Btn>
                </div>
              )}
            </div>
          ))}
